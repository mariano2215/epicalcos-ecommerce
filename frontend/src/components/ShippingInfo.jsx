 ...
 */
export default function ShippingInfo({ subtotal = 0, defaultOpen = false, className = '' }) {
        ...
        <form onSubmit={calcular} className="mt-4 border-t border-white/10 pt-4 space-y-3">
          ...
          {result && !result.error && (
            <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-sm" aria-live="polite">
              ...
              {result.cost === 0 && (
                <p className="text-xs text-emerald-400 mt-2">✓ Tu pedido ya tiene envío gratis a ese destino.</p>
              )}
            </div>
          )}
